 ...
 * 3. Run for root `/favicon.ico` so it can rewrite to market branding assets.
 * 4. Skip `_next`, `_vercel`, and every other dotted file (css, js, images,
...
export function shouldRunShopProxy(pathname: string): boolean {
  ...
  if (path.endsWith('/favicon.ico')) return false
